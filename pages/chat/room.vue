    ...
    <div class="max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
        <div v-for="(message, index) in cleanMessage" :key="index">
            <div v-if="user && user.id === message.sender.id"  class="flex items-start gap-2.5">
                <img class="w-10 h-10 rounded-full inline-flex self-center items-center p-1" :src="message.sender.avatar? message.sender.avatar : 'https://i.pravatar.cc/150?img=3'" alt="Jese image">
...
<div>
    <label for="chat" class="sr-only">Tu mensaje</label>
    <div class="flex items-center px-3 py-2 rounded-lg bg-gray-50 dark:bg-gray-700">
        ...
        <button @click="sendMessage" type="submit" class="inline-flex justify-center p-2 text-blue-600 rounded-full cursor-pointer hover:bg-blue-100 dark:text-blue-500 dark:hover:bg-gray-600">
            <svg class="w-5 h-5 rotate-90 rtl:-rotate-90" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 18 20">
                <path d="m17.914 18.594-8-18a1 1 0 0 0-1.828 0l-8 18a1 1 0 0 0 1.157 1.376L8 18.281V9a1 1 0 0 1 2 0v9.281l6.758 1.689a1 1 0 0 0 1.156-1.376Z"/>
            </svg>
            <span class="sr-only">Enviar message</span>
        </button>
    </div>
</div>
...
onMounted(()=>{
    ...
    channel.bind('sendMessage', function ( data: any ) {
        ...
        if (data.message) {
            const messageExists = cleanMessage.value.find((msg: { id: any; }) => msg.id === data.message.id);
            if (!messageExists) {
